...
export const customerReducer = createReducer(
  ...
  on(
    ...
    (state: CustomerState, { customer, newcustomer }) => ({
      ...state,
      customers: [
        ...state.customers.map((u) => (u !== customer ? u : newcustomer)),
      ],
    })
  ),
  on(CustomerActions.deleteCustomer, (state: CustomerState, { customer }) => ({
    ...state,
    customers: [...state.customers.filter((t) => t !== customer)],
  }))
);
